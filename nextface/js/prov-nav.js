...
(function () {
  ...
  function resolveStep(step) {
    ...
    switch (step) {
      case "1":
        return "/nextface/provisional-entry.html";
      case "2": {
        const target =
          id ||
          pickByStatus(["PROVISIONAL_DRAFT", "OCR_REVIEWING", "ACTIVE"]);
        return target
          ? `/nextface/provisional-review.html?id=${encodeURIComponent(target)}`
          : "/nextface/provisional-dashboard.html";
      }
      case "3": {
        const target =
          id || pickByStatus(["OCR_REVIEWING", "ACTIVE", "PROVISIONAL_DRAFT"]);
        return target
          ? `/nextface/provisional-data-entry.html?id=${encodeURIComponent(target)}`
          : "/nextface/provisional-dashboard.html";
      }
      case "4": {
      ...
      default:
        return "/nextface/provisional-dashboard.html";
    }
  }
